...
liwc=pd.read_table('PAN_liwc_mrc_features_pd.txt',sep=' ',header=None)
print("liwc shape",liwc.shape) # 90,但是实际上是89维，0～89,0是样本号码，1～89才是特征
liwc=liwc.replace('?',0)



mer1=pd.merge(texts_pd,labels_pd,left_on='node_id',right_on='node_id')

print("mer1.shape ",mer1.shape) # should be 7



mer=pd.merge(mer1,liwc,left_on='node_id',right_on=0)  # should be 96
mer=pd.concat([mer.iloc[:,0:7],mer.iloc[:,8:]],axis=1)
...
for time_e in range(0,10):
    ...
    training_with_liwc,testing_with_liwc =train_test_split(mer,test_size=int(train_ratio*mer.shape[0]),shuffle=True)


    # training=training_with_liwc[list(range())]

    texts_train, labels_train = training_with_liwc[['node_text']],training_with_liwc[["trait1","trait2","trait3","trait4","trait5"]]
    texts_test, labels_test=testing_with_liwc[['node_text']],testing_with_liwc[["trait1","trait2","trait3","trait4","trait5"]]
    ...
    conv1=Conv2D(filters=1,kernel_size=(1,w2vDimension),activation="relu")(embedding_layer_reshape)
    conv2=Conv2D(filters=1,kernel_size=(2,w2vDimension),activation="relu")(embedding_layer_reshape)
    conv3=Conv2D(filters=1,kernel_size=(3,w2vDimension),activation="relu")(embedding_layer_reshape)




    max1 = MaxPooling2D(pool_size=(input_length-1+1, 1))(conv1)
    max2 = MaxPooling2D(pool_size=(input_length-2+1, 1))(conv2)
    max3 = MaxPooling2D(pool_size=(input_length-3+1, 1))(conv3)

    concatenate = Concatenate(axis=3)([max1, max2, max3])


    flat=Flatten(name='vectors')(concatenate)
    ...
    train_liwc=training_with_liwc.iloc[:,7:]
    test_liwc=testing_with_liwc.iloc[:,7:]


    x_train_=pd.DataFrame(vectors.predict(data_train))
    train_liwc=train_liwc.reset_index(drop=True)
    x_train=pd.concat([x_train_,train_liwc],axis=1)
    y_train=pd.DataFrame(labels_train)



    x_test_=pd.DataFrame(vectors.predict(data_test))
    test_liwc = test_liwc.reset_index(drop=True)
    x_test=pd.concat([x_test_,test_liwc],axis=1)
