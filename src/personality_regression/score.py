...
data_name="PAN"
model_name="mairesse_de_0.1_0.1_5_10_128_3_0.1_0.3_node2vec"
...
embeddings=pd.read_table(input_feature, sep=" ", header=None,skiprows=[0])
...
for r in r_list:
    ...
    for times in range(score_times):
        count_=count_+1
        print("finished ",count_*1.0/total*100," %.")

        train,test=train_test_split(mer,test_size=1-r,shuffle=True)
        x_train=train.iloc[:,1:feature_size]
        y_train=train.iloc[:,feature_size:]
        x_test=test.iloc[:,1:feature_size]
