...
def _write_walks_to_disk(args):
    num_paths, path_length, alpha, rand, f, importance_content,walk_type,r,p,q = args
    G = __current_graph
    with open(f, 'w') as fout:
        for walk in graph.build_deepwalk_corpus_iter(G=G, num_paths=num_paths, path_length=path_length,
                                                     alpha=alpha, rand=rand,importance_content=importance_content,
                                                     walk_type=walk_type,r=r,p=p,q=q):
            fout.write(u"{}\n".format(u" ".join(v for v in walk)))
    return f


def write_walks_to_disk(G, filebase, num_paths, path_length, alpha=0, rand=random.Random(0), num_workers=cpu_count(),
                        always_rebuild=False,importance_content=None,walk_type="ada_walk",r=0.1,p=0.1,q=0.1):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        for size, file_, ppw in zip(executor.map(count_lines, files_list), files_list, paths_per_worker):
            if always_rebuild or size != (ppw * expected_size):
                args_list.append((ppw, path_length, alpha, random.Random(rand.randint(0, 2 ** 31)), file_,importance_content,walk_type,r,p,q))
            else:
                files.append(file_)
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
